...
# Scales to apply to the WFD rolling regions.
# Should have mean of 1 to keep scaling with other
# regions correct.
up = 1.75
down = 0.25
...
ras, decs = hpid2RaDec(nside, np.arange(even_year_target['r'].size))
dec_split = decs[split_indx]
pad = 1.5  #degrees


indx = np.arange(even_year_target['r'].size)
top_half_wfd = np.where((even_year_target['r'] == 1) & (decs > dec_split+pad))
bottom_half_wfd = np.where((even_year_target['r'] == 1) & (decs < dec_split-pad))
...
observatory, scheduler, observations = fs.sim_runner(observatory, scheduler,
                                                     ...
                                                     filename='rolling_gap%iyrs.db' % years,
